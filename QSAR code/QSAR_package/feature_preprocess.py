...
class correlationSelection(object):
    ...
    def PearsonXY(self,tr_x,tr_y,threshold_low=0):
    ...
    def PearsonXX(self,tr_x,tr_y,threshold_low=0,threshold_up=0.9):
    ...
    def __init__(self,estimator,features_num=1,**params):
        ...
        **params: 学习器的超参数，如SVM有C、gamma、epsilon等，RF有n_estimators、max_leaf_nodes、random_state等"""
        ...
        if type(estimator) == str:
            if estimator == 'SVC':
                self.estimator = SVC(kernel='linear',random_state=0,probability=True)
            elif estimator == 'SVR':
