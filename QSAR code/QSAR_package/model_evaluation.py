...
class modelEvaluator(object):
    ...
    def __init__(self,y_true,y_pred,model_kind=None):
        """参数：
           ...
           """
        if model_kind == None:
            if len(np.unique(y_true))<=2: # 根据标签列的多样性确定是分类还是回归
...
class modeling(object):
    ...
    def CrossVal(self,cv):
        ...
        if type(cv) == int:
            if len(np.unique(self.tr_y))<=2:  # 判断是否为分类数据，如果是，则交叉验证采用分层抽样
    ...
    def ShowResults(self,show_cv=True,make_fig=False):
        ...
        if make_fig:
            ...
            ax.plot(self.tr_y,self.tr_pred_y,marker='o',c='#EE3A8C',linestyle='',markerfacecolor='w',markersize=6)
            ax.plot(self.te_y,self.te_pred_y,marker='x',c='#1874CD',linestyle='',markersize=7)
            ax.plot([axisMin,axisMax],[axisMin,axisMax],'k',lw=1)
            ax.axis([axisMin,axisMax,axisMin,axisMax])
            ax.set_xlabel(r'Experimental pIC$_{50}$ ',fontsize=13)
            ax.set_ylabel(r'Predicted pIC$_{50}$ ',fontsize=13)
