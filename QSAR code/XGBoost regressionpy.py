# -*- coding: utf-8 -*-
"""
Created on Thu Dec  8 20:26:40 2022
...
file = r"D:\academic\01_degree_doctor\01_project\mPGES\06_QSAR\model\update_rdkit\rdkit.csv"
for randx in [8,42,50,65,16,80,100,200,256,512,1024]:
    ...
    for rankN in ['RFR']:
        ...
        for estimatorName in ['XGBR']:
            grid = gridSearchPlus(grid_estimatorName=estimatorName, fold=5, repeat=2,scoreThreshold=-0.6)
            grid.FitWithFeaturesNum(tr_ranked_x, tr_y,features_range=(10,41))   # RFE ranking
            ...
            path = r"D:\academic\01_degree_doctor\01_project\mPGES\06_QSAR\model\update_rdkit\RDK2_pred_{}_{}_{}.csv".format(randx,notes1,estimatorName)
            pred_results.to_csv(path,index=False)
            with open(r"D:\academic\01_degree_doctor\01_project\mPGES\06_QSAR\model\update_rdkit\RDK2_des_{}_{}_{}.csv".format(notes1,randx,estimatorName),'w') as fobj:
                fobj.write('\n'.join(grid.best_features))
            model.ShowResults(show_cv=True,make_fig=False)
            model.SaveResults(file[:-4]+'_results2.csv',notes='{},split_seed={},gridCV=5'.format(notes1,randx),save_model=True)
